...
: "${GHCR_TOKEN:?GHCR_TOKEN is required for the remote registry pull}"
...
# Authenticate the isolated deploy user without persisting registry credentials.
# The app receives a sanitized environment file so deployment-plane credentials
# can never leak into either the schema migration or the long-lived container.
