...
const INTENT_RATE_WINDOW_MS = 10_000;

type MatchPhaseBucket = "early" | "mid" | "late";
...
export class GameServer {
  ...
  private intentRateState = new Map<
    ClientID,
    {
      lastByType: Map<string, number>;
      windowStart: number;
      windowCounts: Map<string, number>;
      dropped: number;
    }
  >();
  private intentFunnel: Record<MatchPhaseBucket, Record<string, number>> = {
    early: {},
    mid: {},
    late: {},
  };
  ...
  public updateGameConfig(gameConfig: Partial<GameConfig>): void {
    ...
    if (gameConfig.vaultSitesEnabled !== undefined) {
    ...
    if (gameConfig.vaultRuntimeExperiment !== undefined) {
      this.gameConfig.vaultRuntimeExperiment =
        gameConfig.vaultRuntimeExperiment;
    }
    if (gameConfig.vaultWeeklyMutator !== undefined) {
      this.gameConfig.vaultWeeklyMutator = gameConfig.vaultWeeklyMutator;
    }
    if (gameConfig.contractCardsEnabled !== undefined) {
      this.gameConfig.contractCardsEnabled = gameConfig.contractCardsEnabled;
    }
    if (gameConfig.nationGoldTroopEmphasis !== undefined) {
      this.gameConfig.nationGoldTroopEmphasis =
        gameConfig.nationGoldTroopEmphasis;
    }
  }
            ...
            switch (stampedIntent.type) {
              ...
              case "toggle_pause": {
                ...
                if (stampedIntent.paused) {
                  ...
                  this.addIntent(stampedIntent, client.clientID);
                  this.endTurn();
                  this.isPaused = true;
                } else {
  ...
  private addIntent(intent: StampedIntent, clientID?: ClientID) {
    if (clientID && !this.shouldAcceptIntent(clientID, intent)) {
      return;
    }
  ...
  private intentRateKey(intent: StampedIntent): string {
    if (intent.type === "vault_convoy_command") {
      return `${intent.type}:${intent.command}`;
    }
    if (intent.type === "defense_factory_command") {
      return `${intent.type}:${intent.command}`;
    }
    if (intent.type === "vault_role_ping") {
      return `${intent.type}:${intent.ping}`;
    }
    return intent.type;
  }

  private intentMinIntervalMs(intent: StampedIntent): number {
    switch (intent.type) {
      case "vault_convoy_command":
        if (intent.command === "escort") return 280;
        if (intent.command === "reroute_safest") return 180;
        return 220;
  ...
  private intentMaxPerWindow(intent: StampedIntent): number {
    switch (intent.type) {
      case "vault_convoy_command":
        return 20;
      case "defense_factory_command":
  ...
  private shouldAcceptIntent(
    ...
    if (!state) {
      ...
      this.intentRateState.set(clientID, state);
    }

    const rateKey = this.intentRateKey(intent);
    const minInterval = this.intentMinIntervalMs(intent);
    if (minInterval > 0) {
      const last = state.lastByType.get(rateKey) ?? 0;
      if (now - last < minInterval) {
        state.dropped += 1;
        if (state.dropped % 10 === 1) {
          this.log.warn("intent blocked by cooldown rate limiter", {
            gameID: this.id,
            clientID,
            type: rateKey,
      ...
      state.lastByType.set(rateKey, now);
    }

    if (now - state.windowStart >= INTENT_RATE_WINDOW_MS) {
      state.windowStart = now;
      state.windowCounts.clear();
    }
    const maxPerWindow = this.intentMaxPerWindow(intent);
    const count = state.windowCounts.get(rateKey) ?? 0;
    if (count >= maxPerWindow) {
      state.dropped += 1;
      if (state.dropped % 10 === 1) {
        this.log.warn("intent blocked by window rate limiter", {
          gameID: this.id,
          clientID,
          type: rateKey,
          dropped: state.dropped,
    ...
    state.windowCounts.set(rateKey, count + 1);
    return true;
  }

  private phaseBucketForTurn(turnNumber: number): MatchPhaseBucket {
    const elapsedSec = Math.floor(
      (turnNumber * this.config.turnIntervalMs()) / 1000,
    );
    if (elapsedSec < 300) return "early";
    if (elapsedSec < 900) return "mid";
    return "late";
  }

  private recordIntentFunnel(intent: StampedIntent): void {
    const bucket = this.phaseBucketForTurn(this.turns.length);
    const key = intent.type;
    const current = this.intentFunnel[bucket][key] ?? 0;
    this.intentFunnel[bucket][key] = current + 1;
  }
      ...
      return [
        {
          ...
          firstVaultPressureTick: toTick(vault?.firstVaultPressureTick),
          firstBreachOpenTick: toTick(vault?.firstBreachOpenTick),
          decisiveDeliveryTick: toTick(vault?.decisiveDeliveryTick),
          vaultBreachVictoryTick: toTick(vault?.vaultBreachVictoryTick),
