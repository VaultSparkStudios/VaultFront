...
- Verification: exact evidence head `be5a05986e385fccc86132565f0eed5d9e541a13` passed Release, brief-format, build, lint, format, bundle, and all 232 files / 1,221 tests, but CI failed after the registry updated `GHSA-2v37-7h3g-55p8` against Nano ID 3.3.16. Trust review approved the signed 3.3.18 release, which completes the async native zero-size guard omitted by 3.3.17. Local focused ID tests pass 10/10; production audit, supply-chain scan, build, Playwright, and visual receipt pass pending successor exact provider proof. DOMPurify remains pinned to trust-approved 3.4.13.
