...
- Dependency truth: exact executable-mode SHA `a1f28f82` passed E2E, Release, brief-format, build, lint, format, bundle, and all 1,221 tests; CI failed only because the registry began reporting `GHSA-55q2-fjhq-7xh7` during the run. The official advisory names DOMPurify 3.4.13 as first patched, its Cure53 tag is PGP-verified, the npm identity/integrity/license are coherent, and the exact patch now passes local production audit, incident scan, and build. Successor provider proof remains pending.
