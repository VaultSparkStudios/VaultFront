...
const REQUIRED_BLOCKS = [
  ...
  {
    label: "HUMAN or RELEASE PRESSURE block",
    pattern: /╔══\s*(?:HUMAN|RELEASE) PRESSURE/,
    severity: "recommended",
  },
];
...
function semanticContradictions(body) {
  const contradictions = [];
  const greenUnknown = body
    .split(/\r?\n/)
    .find((line) => /✓/.test(line) && /\bunknown\b/i.test(line));
  if (greenUnknown) {
    contradictions.push(
      "Unknown evidence is rendered green; unknown must remain warning or blocked.",
    );
  }
  if (/LAST SESSION\s*\(S\?\)/i.test(body)) {
    contradictions.push(
      "LAST SESSION uses S? instead of a resolved session identity.",
    );
  }
  const placeholderTests = /Tests\s+-\s*(?:║|$)/m.test(body);
  const placeholderDeploy = /Deploy\s+-\s*(?:║|$)/m.test(body);
  if (placeholderTests || placeholderDeploy) {
    contradictions.push(
      "Placeholder test or deploy evidence is not a release signal.",
    );
  }
  if (placeholderTests && placeholderDeploy) {
    contradictions.push(
      "Test and deploy evidence have different producers; stamp them separately.",
    );
  }
  for (const match of body.matchAll(/Last active:\s*(\d+)d/gi)) {
    const days = Number(match[1]);
    if (days > 3650) {
      contradictions.push(
        `Last active reports ${days} days; reject likely non-date chronology input.`,
      );
    }
  }
  const contextBlock =
    body.match(/╔══\s*CONTEXT METER[^\n]*\n([\s\S]*?)╚[^\n]*╝/)?.[1] ?? "";
  const pctMatch = contextBlock.match(/(\d{1,3})%\s+used/);
  const tokenMatch = contextBlock.match(/([\d,]+)\s*\/\s*([\d,]+)\s+tok/i);
  if (pctMatch && tokenMatch) {
    const rendered = Number(pctMatch[1]);
    const used = Number(tokenMatch[1].replaceAll(",", ""));
    const limit = Number(tokenMatch[2].replaceAll(",", ""));
    const computed = limit > 0 ? Math.round((used / limit) * 100) : NaN;
    if (!Number.isFinite(computed) || Math.abs(rendered - computed) > 1) {
      contradictions.push(
        `Context token arithmetic contradicts itself: ${rendered}% rendered beside ${used}/${limit} tokens (computed ${Number.isFinite(computed) ? computed + "%" : "invalid"}).`,
      );
    }
  }

  const forecastBlock =
    body.match(/╔══\s*SIL FORECAST[^\n]*\n([\s\S]*?)╚[^\n]*╝/)?.[1] ?? "";
  const numericForecast = /Projected:\s*\d+\s*\/\s*1000/i.test(forecastBlock);
  const evidenceCount = Number(
    forecastBlock.match(/Evidence:\s*(\d+)\s+parsed SIL sessions?/i)?.[1] ?? 0,
  );
  if (numericForecast && evidenceCount <= 0) {
    contradictions.push(
      "SIL forecast presents a numeric projection with zero parsed SIL evidence.",
    );
  }
...
export function validateStartupBrief(body) {
  const findings = {
    ...
    bodyShape: null,
    staleBrief: null,
    semanticContradictions: semanticContradictions(body),
  };
